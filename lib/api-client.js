...
export const jobApi = {
  ...
  // Faz 8 Block D — sponsored listing checkout.
  checkout: (id, durationDays) => api.post(`/api/jobs/${id}/checkout`, { durationDays }),
}
...
export const favoritesApi = {
  list: (limit) => api.get('/api/favorites/me', { query: { limit } }),
  isFavorited: (userId) => api.get(`/api/favorites/me/${userId}`),
  add: (userId) => api.post(`/api/favorites/${userId}`),
  remove: (userId) => api.delete(`/api/favorites/${userId}`),
}
...
export const subscriptionApi = {
  ...
  change: (plan, billing = 'monthly') => api.post('/api/subscriptions/change', { plan, billing }),
}
...
export const adminApi = {
  ...
  updatePlan: (id, patch) => api.patch(`/api/admin/plans/${id}`, patch),
  payments: (limit) => api.get('/api/admin/payments', { query: { limit } }),
  refundsList: (limit) => api.get('/api/admin/refunds', { query: { limit } }),
  disputes: (limit) => api.get('/api/admin/disputes', { query: { limit } }),
  createRefund: (body) => api.post('/api/admin/refunds', body),
}
...
export const stripeApi = {
  ...
  config: () => api.get('/api/stripe/config'),
  checkout: ({ plan, billing }) => api.post('/api/stripe/checkout', { plan, billing }),
  // On-site Payment Element: returns { clientSecret, subscriptionId, status, mode }.
  subscribe: ({ plan, billing }) => api.post('/api/stripe/subscribe', { plan, billing }),
  portal: (returnUrl) => api.post('/api/stripe/portal', returnUrl ? { returnUrl } : {}),
}
